...
// ---------------------------------------------------
// ⚙️ 報酬設定データ（5勝=メダル / 10勝=王冠 / 20勝=トロフィー）
// ---------------------------------------------------
const REWARD_CONFIG = {
  5: {
    baseClass: 'reward-medal-icon', // 🏅 メダル用
    easy:   { colorClass: 'medal-bronze', name: '銅メダル' },
    medium: { colorClass: 'medal-silver', name: '銀メダル' },
    hard:   { colorClass: 'medal-gold',   name: '金メダル' }
  },
  10: {
    baseClass: 'reward-crown-icon', // 👑 王冠用
    easy:   { colorClass: 'crown-bronze', name: '銅の王冠' },
    medium: { colorClass: 'crown-silver', name: '銀の王冠' },
    hard:   { colorClass: 'crown-gold',   name: '金の王冠' }
  },
  20: {
    baseClass: 'reward-trophy-icon', // 🏆 トロフィー用（追加！）
    easy:   { colorClass: 'trophy-bronze', name: '銅のトロフィー' },
    medium: { colorClass: 'trophy-silver', name: '銀のトロフィー' },
    hard:   { colorClass: 'trophy-gold',   name: '金のトロフィー' }
  }
};
  ...
  <div id="rewardModalOverlay" class="reward-modal-overlay">
    <div class="reward-modal-box">
      ...
      <div class="reward-medal-container">
        ...
        <h2 style="margin: 5px 0; font-size: 22px;">${winCount}連勝達成！</h2>
        <p style="font-size: 12px; color: #ccc; margin: 0;">${diffData.name}を獲得しました！</p>

        <button class="reward-close-btn" onclick="closeRewardModal()">受け取る！</button>
      </div>
    </div>
  `;
